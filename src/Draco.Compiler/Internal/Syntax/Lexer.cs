...
internal sealed class Lexer
{
    ...
    public SyntaxToken Lex()
    {
        ...
        case ModeKind.MultiLineInterpolation:
        {
            ...
            if (this.tokenBuilder.Type != TokenKind.InterpolationEnd) this.ParseTrailingTriviaList();
            break;
        }
        ...
        case ModeKind.MultiLineString:
            ...
            if (this.tokenBuilder.Type == TokenKind.InterpolationStart) this.ParseTrailingTriviaList();
            break;
    ...
    private Unit LexNormal()
    {
        ...
        {
            this.Advance(length);
            this.tokenBuilder.SetType(tokenKind);
            return default;
        }
        ...
        {
            this.tokenBuilder.SetType(TokenKind.EndOfInput);
            return default;
        }
        ...
        if (char.IsDigit(ch))
        {
            ...
            if (this.Peek(offset) == '.' && char.IsDigit(this.Peek(offset + 1)))
            {
                ...
                this.tokenBuilder
                    .SetType(TokenKind.LiteralFloat)
            ...
            this.tokenBuilder
                .SetType(TokenKind.LiteralInteger)
        ...
        if (IsIdent(ch))
        {
            ...
            var tokenKind = ident switch
            {
                ...
                var _ when ident.Span.SequenceEqual("from") => TokenKind.KeywordFrom,
        ...
        if (ch == '\'')
        {
            ...
            this.tokenBuilder
                .SetType(TokenKind.LiteralCharacter)
                .SetText(text)
                .SetValue(resultChar);
            return default;
        }
    ...
    start:
        ...
        if (ch == '\0' && offset > 0)
        {
            ...
            this.tokenBuilder
                .SetType(TokenKind.StringContent)
        ...
        if (ch == '\\')
        {
            ...
            if (this.Peek(offset + mode.ExtendedDelims + 1) == '{')
            {
                ...
                if (offset == 0)
                {
                    ...
                    this.tokenBuilder
                        .SetType(TokenKind.InterpolationStart)
            ...
            if (mode.Kind == ModeKind.MultiLineString)
            {
                ...
                if (this.TryParseNewline(offset2 + whiteCharOffset, out var length))
                {
                    ...
                    if (offset == 0)
                    {
                        ...
                        this.tokenBuilder
                            .SetType(TokenKind.StringNewline)
            ...
            else
            {
                ...
                if (offset == 0)
                {
                    ...
                    if (this.Peek(endOffset) == '"' && this.Peek(endOffset + 1) == '"' && this.Peek(endOffset + 2) == '"')
                    {
                        ...
                        this.tokenBuilder
                            .SetType(TokenKind.MultiLineStringEnd)
    ...
    private bool TryParseTrivia([MaybeNullWhen(false)] out SyntaxTrivia result)
    {
        ...
        if (ch == '/' && this.Peek(1) == '/')
        {
            // Line Comment
            var offset = 2;
            var commentType = TriviaKind.LineComment;
            // Documentation Comment
            if (this.Peek(2) == '/')
            {
                offset = 3;
                commentType = TriviaKind.DocumentationComment;
            }
