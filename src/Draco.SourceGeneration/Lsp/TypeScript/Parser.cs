...
internal sealed class Parser
{
    ...
    start:
        ...
        return peek.Kind switch
        {
            ...
            _ => throw new InvalidOperationException($"unexpected token {peek.Text}"),
        };
    }
    ...
    private Expression ParseAtomExpression()
    {
        ...
        throw new InvalidOperationException($"unexpected token {this.Peek().Text}");
    }
    ...
    {
        var peek = this.Peek();
        if (!IsNameLike(peek.Kind)) throw new InvalidOperationException($"expected name, but got {peek.Text}");
        return this.Advance();
    }
    ...
    private Token Expect(TokenKind type)
    {
        ...
        {
            throw new InvalidOperationException($"Expected token {type}, but got {this.Peek().Text}");
        }
        return result;
    }
